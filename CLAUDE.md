...
**Optional env vars:** `DB_PATH`, `ALLOWED_ORIGIN`, `DT_URL`, `SPV_URL`, `LB_URL`, `PORT`, `LB_EXPORT_TOKEN` (shared secret for the public export endpoints `/api/contacts/export` + `/api/staff/export`; when set, callers must send it as the `X-Export-Token` header or `?token=` — Leaderboard must be updated to send it. When unset the endpoints stay publicly readable and the server logs a startup warning), SMTP vars (`SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASS`, `SMTP_FROM`), `ANTHROPIC_API_KEY` (enables the "Read receipt to autofill" button on expenses — without it, `POST /api/expenses/extract` returns 503), `PAYMENTS_KEK` (32-byte base64; encrypts Stripe/Mercury credentials in `firm_payment_config` **and** TOTP secrets in `users.totp_secret`. Generate with `node -e "console.log(require('crypto').randomBytes(32).toString('base64'))"`. Without it, the Payments settings tab loads read-only, `PUT /api/firm/payments` returns 503, and the 2FA endpoints return 503 — the rest of the app runs normally). See `PAYMENTS_DESIGN.md` for the full payments architecture.
