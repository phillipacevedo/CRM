...
**Optional env vars:** `DB_PATH`, `ALLOWED_ORIGIN`, `DT_URL`, `SPV_URL`, `LB_URL`, `PORT`, SMTP vars (`SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASS`, `SMTP_FROM`), `ANTHROPIC_API_KEY` (enables the "Read receipt to autofill" button on expenses — without it, `POST /api/expenses/extract` returns 503).
